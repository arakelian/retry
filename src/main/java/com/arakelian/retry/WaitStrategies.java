...
public final class WaitStrategies {
    ...
    private static final class CompositeWaitStrategy implements WaitStrategy {
        ...
        public long computeSleepTime(final Attempt failedAttempt) {
            long waitTime = 0L;
            for (final WaitStrategy waitStrategy : waitStrategies) {
    ...
    private static final class ExceptionWaitStrategy<T extends Throwable> implements WaitStrategy {
        ...
        public long computeSleepTime(final Attempt lastAttempt) {
            if (lastAttempt.hasException()) {
                final Throwable cause = lastAttempt.getExceptionCause();
    ...
    private static final class ExponentialWaitStrategy implements WaitStrategy {
        ...
        public ExponentialWaitStrategy(final long multiplier, final long maximumWait) {
            ...
            Preconditions.checkArgument(
                    ...
                    "multiplier must be < maximumWait but is %s",
                    multiplier);
    ...
    private static final class RandomWaitStrategy implements WaitStrategy {
        ...
        public long computeSleepTime(final Attempt failedAttempt) {
            ...
            final long positive = rand == Long.MAX_VALUE ? 0 : Math.abs(rand);
            final long t = Math.abs(positive) % (maximum - minimum);
            return t + minimum;
        }
    }
     ...
     * The {@code function} determines how the sleep time should be calculated for the given
     ...
     *
     * @param multiplier
     *            multiply the wait time calculated by this
     * @param maximumTime
    ...
    public static WaitStrategy exponentialWait(
            ...
            @Nonnull final TimeUnit maximumTimeUnit) {
     ...
     *            the time to sleep
     * @param timeUnit
     *            the unit of the time to sleep
     * @return a wait strategy that sleeps a fixed amount of time
     * @throws IllegalStateException
     *             if the sleep time is &lt; 0
     */
    public static WaitStrategy fixedWait(final long sleepTime, @Nonnull final TimeUnit timeUnit)
            throws IllegalStateException {
     ...
     * @param initialSleepTimeUnit
     *            the unit of the initial sleep time
     * @param increment
     *            the increment added to the previous sleep time after each failed attempt
     * @param incrementTimeUnit
     *            the unit of the increment
    ...
    public static WaitStrategy incrementingWait(
            ...
            @Nonnull final TimeUnit initialSleepTimeUnit,
            final long increment,
            @Nonnull final TimeUnit incrementTimeUnit) {
     ...
     *            Wait strategies that need to be applied one after another for computing the sleep
     *            time.
     * @return A composite wait strategy
     */
    public static WaitStrategy join(final WaitStrategy... waitStrategies) {
        Preconditions.checkState(waitStrategies.length > 0, "Must have at least one wait strategy");
        final List<WaitStrategy> waitStrategyList = Lists.newArrayList(waitStrategies);
     ...
     * @throws IllegalStateException
     *             if the maximum sleep time is &lt;= 0.
     */
    public static WaitStrategy randomWait(final long maximumTime, @Nonnull final TimeUnit timeUnit) {
        Preconditions.checkNotNull(timeUnit, "The time unit may not be null");
     ...
     * @param minimumTimeUnit
     *            the unit of the minimum time
     * @param maximumTime
     *            the maximum time to sleep
     * @param maximumTimeUnit
     *            the unit of the maximum time
     * @return a wait strategy with a random wait time
     * @throws IllegalStateException
     *             if the minimum sleep time is &lt; 0, or if the maximum sleep time is less than
     *             (or equals to) the minimum.
     */
    public static WaitStrategy randomWait(
            final long minimumTime,
            @Nonnull final TimeUnit minimumTimeUnit,
            final long maximumTime,
            @Nonnull final TimeUnit maximumTimeUnit) {
